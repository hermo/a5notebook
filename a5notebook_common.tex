...
\def\dotsize{0.13}      % Dot radius (mm)
...
\def\sewingmargin{12}     % Outer sewing holes stay this far from the top/bottom (mm)
\def\sewingpointsize{0.3} % Radius of the awl-punch guide points (mm)
...
\newcommand{\makepage}[5]{%
    \begin{scope}[shift={(#1,#2)}]
        ...
        \ifnum#3=3% Dots
            ...
            % top and bottom dots both sit exactly \dotspacing from their
            % respective border, so the grid fills the whole page instead
            % of stopping short at \vbottommargin - it's fine for dots to
            % overlap the page number since they're tiny.
            \pgfmathtruncatemacro{\ndotcols}{floor((148.5-\dotmarginright-\dotmarginleft)/\dotspacing)}
            \pgfmathtruncatemacro{\ndotrows}{floor((210-2*\dotspacing)/\dotspacing)}
            \foreach \i in {0,...,\ndotcols} {
                \foreach \j in {0,...,\ndotrows} {
                    \ifnum#5=0
        ...
        \ifdim#1<74mm % Left page
            \node[black!100] at (\marginleft mm,7mm) {\small\bfseries #4};
        \else % Right page
            \node[black!100] at ({148.5mm-\marginright mm},7mm) {\small\bfseries #4};
        \fi
    \end{scope}
}

% Binding/sewing guide on the spine between the two facing pages: five tiny
% points marking where to punch holes with an awl for a 5-hole pamphlet
% stitch - one at the vertical center, one pair at \sewingmargin from the
% top/bottom edges, and one pair at the midpoints between those and the
% center.
% #1 = spine x-offset expression
\newcommand{\spinemarker}[1]{%
    \pgfmathsetmacro{\sewtop}{\sewingmargin}
    \pgfmathsetmacro{\sewbottom}{210-\sewingmargin}
    \pgfmathsetmacro{\sewcenter}{105}
    \pgfmathsetmacro{\sewuppermid}{(\sewtop+\sewcenter)/2}
    \pgfmathsetmacro{\sewlowermid}{(\sewcenter+\sewbottom)/2}
    \foreach \sewy in {\sewtop,\sewuppermid,\sewcenter,\sewlowermid,\sewbottom} {
        \fill[black!45] ({#1},\sewy mm) circle[radius=\sewingpointsize mm];
    }
}
